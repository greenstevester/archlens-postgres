...
The file where human intent lives, and what the interesting checks compare against. Three parts: `domains[]` (key, title, blurb, `tenant_scoped`, `tables[]`), `conventions[]` (printed in the docs, not checked), and `assertions` (`tenant_table`, `tenant_column`, `require_rls`, `global_tables`, `singleton_tables`, `cardinality[]`, `natural_keys[]`, `exclusive_arcs[]`, `require_relationship_notes`). A `cardinality[]` entry may carry a `why`; the docs print it beside the relationship, and with `require_relationship_notes` every foreign key without one becomes an `undocumented-relationship` note. Every table must be claimed by exactly one domain; otherwise `domain-coverage` raises an error and the run exits 1. That gate is what keeps the docs accurate as the schema grows. See `examples/narratives.json` for the shape.
